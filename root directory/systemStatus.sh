...
read prev_status < /root/prevPowerStatus.txt
read prev_battery_level < /root/prevBatteryLevel.txt
...
then
 echo "${battery_level}" > "/root/prevBatteryLevel.txt"
...
then
 echo "${ups_status}" > "/root/prevPowerStatus.txt"
...
read prev_IntStatus < /root/prev_IntStatus.txt
/bin/ping -c 3 $SERVERIP > /dev/null 2>&1
if [ $? -ne 0 ]
then
	if [ "Not Available" != "${prev_IntStatus}" ];
	then
 		echo "Not Available" > "/root/prev_IntStatus.txt"
	...
	if [ "Available" != "${prev_IntStatus}" ];
        then
                echo "Available" > "/root/prev_IntStatus.txt"
...
read prev_ISP < /root/prev_ISP.txt
if [ "${ISPsource}" != "${prev_ISP}" ];
then
 echo "${ISPsource}" > "/root/prev_ISP.txt"
